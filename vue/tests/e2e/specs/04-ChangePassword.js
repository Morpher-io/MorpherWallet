...
describe('Change Password', () => {
	...
	it('error if bad first password', () => {
		...
		cy.get('[data-cy=oldPassword]').type('123456');

		cy.get('[data-cy=newPassword]').type(newPassword);

		cy.get('[data-cy=newPasswordRepeat]').type(newPassword);
	...
	it('change password', () => {
		...
		cy.waitUntil(() => cy.get('[data-cy=confirmed]').contains('Saved'));
	});
	...
	it('change password back to original', () => {
		...
		cy.get('[data-cy=oldPassword]').type(newPassword);

		cy.get('[data-cy=newPassword]').type(password);

		cy.get('[data-cy=newPasswordRepeat]').type(password);
