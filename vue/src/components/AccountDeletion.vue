<template>
  <div>
    ...
    <div class="settings-data user-details has-text-left">
      <div @click="changeMethod(0)" :class="{
    ...
    <div class="settings-data user-details mt-2 has-text-left">
      <div @click="changeMethod(1)" :class="{
        'details select-custom': true,
    ...
    <p class="reset-line-height is-size-7 reset-mt">
      ...
      <span v-if="currentMethod === 1">{{ $t('delete.DO_NOT_KNOW_KEYS') }}
    ...
    <button data-cy="confirmDeleteButton" @click="deleteAccount()"
      class="button is-green big-button is-login transition-faster mt-5" :disabled="!input">
