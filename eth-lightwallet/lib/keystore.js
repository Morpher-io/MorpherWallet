...
KeyStore.generateRandomSeed = function (extraEntropy) {
  ...
  if (extraEntropy === undefined) {
    ...
    const entBuf = new Buffer.from(extraEntropy);
...
KeyStore._computePubkeyFromPrivKey = function (privKey, curve) {
  ...
  const privateKeyBase64 = (new Buffer.from(privKey, 'hex')).toString('base64');
  ...
  const pubKeyHex = (new Buffer.from(pubKeyBase64, 'base64')).toString('hex');
