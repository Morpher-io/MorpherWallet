<template>
  <div>
    ...
    <img
      v-if="
        this.authenticator && (this.qrCode !== '' || this.qrCode !== undefined)
      "
...
export default {
  ...
  methods: {
    async formSubmitChange2FA(e) {
      ...
      try {
        if (!this.email && this.authenticator && !this.authenticatorConfirmed) {
          ...
          const nonce = (
            await change2FAMethods(email, null, this.email, this.authenticator)
          ).nonce;

          const password = window.sessionStorage.getItem("password");
          const encryptedSeed = window.localStorage.getItem("encryptedSeed");
          const keystore = await getKeystoreFromEncryptedSeed(
            JSON.parse(encryptedSeed),
            password
          );
          await change2FAMethods(
            email,
            keystore[0].sign("authentication" + "_" + nonce),
            this.email,
            this.authenticator
          );
          // const self = this;
    ...
    async confirmAuthenticator() {
      ...
      this.authenticatorConfirmed = (
        await verifyAuthenticatorCode(email, this.authenticatorCode)
      ).verified;
  ...
  async mounted() {
    ...
    try {
      ...
      if (twoFAMethods.authenticator !== undefined)
        this.authenticator = twoFAMethods.authenticator;
      if (twoFAMethods.authenticatorConfirmed !== undefined)
        this.authenticatorConfirmed = twoFAMethods.authenticatorConfirmed;
    } catch (e) {
