			...
			const watchTimer = setInterval(async function() {
				try {
					...
					if (uriRegex.test(win.location)) {
						...
						try {
							const encryptedSeedVK = await recoverVKSeed(params.access_token, this.walletEmail);
							...
							const encryptedSeedPassword = await changePasswordEncryptedSeed(encryptedSeedVK, params.user_id, newPasswordForLocalStorage);
							...
							alert("Your account wasn't found with VK recovery, create one with username and password first");
						}
					}
				} catch (e) {
					console.log(e);
